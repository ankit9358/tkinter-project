...
def order():
    ...
    def action():
        ...
        if(q1!=0):
            ...
            row1=cur.fetchone()
            cur.execute("INSERT INTO bill(item,price,quantity,sum_) VALUES (%s,%s,%s,%s)",(i1,row1[0],q1,row1[0]*q1))
        if(q2!=0):
            query="SELECT price FROM menu WHERE item = %s"
            cur.execute(query,(i2,))
            row2=cur.fetchone()
            cur.execute("INSERT INTO bill(item,price,quantity,sum_) VALUES (%s,%s,%s,%s)",(i2,row2[0],q2,row2[0]*q2))
        if(q3!=0):
            query="SELECT price FROM menu WHERE item = %s"
            cur.execute(query,(i3,))
            row3=cur.fetchone()
            cur.execute("INSERT INTO bill(item,price,quantity,sum_) VALUES (%s,%s,%s,%s)",(i3,row3[0],q3,row3[0]*q3))
        if(q4!=0):
            query="SELECT price FROM menu WHERE item = %s"
            cur.execute(query,(i4,))
            row4=cur.fetchone()
            cur.execute("INSERT INTO bill(item,price,quantity,sum_) VALUES (%s,%s,%s,%s)",(i4,row4[0],q4,row4[0]*q4))
        if(q5!=0):
            query="SELECT price FROM menu WHERE item = %s"
            cur.execute(query,(i5,))
            row5=cur.fetchone()
            cur.execute("INSERT INTO bill(item,price,quantity,sum_) VALUES (%s,%s,%s,%s)",(i5,row5[0],q5,row5[0]*q5))
        if(q6!=0):
            query="SELECT price FROM menu WHERE item = %s"
            cur.execute(query,(i6,))
            row6=cur.fetchone()
            cur.execute("INSERT INTO bill(item,price,quantity,sum_) VALUES (%s,%s,%s,%s)",(i6,row6[0],q6,row6[0]*q6))
        if(q7!=0):
            query="SELECT price FROM menu WHERE item = %s"
            cur.execute(query,(i7,))
            row7=cur.fetchone()
            cur.execute("INSERT INTO bill(item,price,quantity,sum_) VALUES (%s,%s,%s,%s)",(i7,row7[0],q7,row7[0]*q7))
