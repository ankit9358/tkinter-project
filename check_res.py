...
def check_r():
    ...
    def action():
        ...
        try:
            ...
            tree.heading("#6",text="From")
            tree.column("#6",width=80)
            tree.heading("#7",text="To")
            tree.column("#7",width=80)
            tree.heading("#8",text="Tables")
            ...
            cancel_label=ttk.Label(win,text='Cancel Reservetion',font=("Times New Roman",20))
            cancel_label.pack()
            def action2():
                cur.execute("DELETE FROM customer WHERE mobile = '%s'"%cos_mob)
                #print(cos_mob)
                row=cur.fetchone()
                con.commit()
                #con.close()
                messagebox.showinfo("Message", "Details of customer has been removed successfylly! ")
                win.destroy()
            def action3():
                #con.close()
                win.destroy()
